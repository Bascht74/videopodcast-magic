...
What was measured -- hit rates, run times, distributions, comparisons
-- is in [What was measured](measurements.md).
...
worker process switches that off first and does not run at all where
it cannot find the switch.
...
the same thing, not by how alike they are. Level and correlation both
...
The recording is judged as a whole and not block by block. Blocks are
read one at a time because they do not all fit in memory at once. A
block is read in one pass and taken apart afterwards, rather than
...
ffmpeg's own result depends on the output format: writing integers it
scales the matrix down against clipping and the level comes out right
after all, writing floats it does not. The same call is correct in one
place and 3 dB out in the next. Its own conversion uses an equal-power
law.
...
The wide shot comes in at a sentence beginning within +/-2 s, else at
a clause break within +/-2 s, else at a sentence beginning within
+/-5 s, else at a clause break within +/-5 s. Both directions.

The exact spot comes out of the audio: in a window of +/-0.5 s around
the target the dip, threshold p5 + 0.30 * (p95 - p5) of the 10 ms
levels in that window itself, chosen by width - 0.5 * distance. A fixed
dB threshold fails on quiet material.
