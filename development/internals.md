...
* `ui/` **4333** -- the window and everything it shows, asks or offers,
  less what a subject has taken to stand beside its own logic
* `speakers/` **3710** -- who speaks and when: the separation itself,
...
* `player/` **3112** -- the moving picture: the player, the cut band,
  the log view, the player menu, and the hush that stops one player
  when the other starts
* `cut/` **3006** -- who is on camera when, and what carries it out
  of here, and why a file can be the wide shot or cannot; the voices
  reach it named, out of `speakers/`
* `resolve/` **2974** -- the DaVinci Resolve project, timelines, colour,
  markers, and the box in the window that says whether Resolve answers
* `material/` **2853** -- channels, chains, continuation files, what a
  track is made of, and the channel rows the window shows of it
* `pipeline/` **2185** -- the chain the recordings run until the camera
  files are written
* `bearings/` **1977** -- where each file and each voice sits, and how
...
* `auphonic/` **1661** -- the sending to auphonic.com and the fetching
...
* `setup/` **1089** -- finding ffmpeg, installing a missing module,
  keeping the key
* `hearing/` **1053** -- decoding, envelopes, bands, phase, aligning
  audio to video
...
* `herald/` **615** -- the progress bar, the stages, the console and log
  redirection
* `project/` **585** -- the program's own project file: writing
...
* `language/` **365** -- a .po file per language and the reader that
  looks one up
...
which stood 800 lines *above* that seam, answered `AttributeError:
'Program' object has no attribute 'chain_fill_in'`, while the same
name at the call site answered the very function the window held.
`chain_fill_in` moved into `filelist/` itself on 7.9.2026 and is an
ordinary name there now; `join_box_fill`, which stands below the seam,
is the one that goes this way today.
