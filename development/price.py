...
five terms below are what it misses. Every one is read out of the tree
...
def main(argv):
    ...
    paid, net = len(B) + len(C) + len(E), len(B) + len(C) + len(E) - len(F)
    ...
    print("  ----")
    print("  paid = B+C+E = %d      net = paid - F = %+d" % (paid, net))
