...
# And where each of them lies, in tests/ or in the folder of its piece.
PLACES = dict(overview.test_places(HERE))
    ...
    for i, line in enumerate(text.splitlines(), 1):
        for hit in LOOKS_LIKE.findall(line):
            ...
            if path.startswith("docs/notes") or path in NOT_SHIPPED:
                continue
            # A test is asked of the suite: the one set aside is still one
            # of ours, and a skill naming it is not naming a dead file.
            # By its bare name, or by the path it lies at -- tests/ and
            # the name alone points nowhere once it lies in a folder.
            base = os.path.basename(path)
            test = base[:-len("_test.py")]
            if base.endswith("_test.py") and test in SUITE and path in (
                    base, "tests/" + PLACES.get(test, base)):
                continue
