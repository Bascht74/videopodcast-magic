...
# one joins them.
mute = []
# Every folder of the suite, and only what lies on this machine: a test
# the builder set aside is not read here, as it never was.
for _name, path in overview.test_places(HERE):
    name = path.rpartition("/")[2]
    whole = os.path.join(HERE, *path.split("/"))
    if not os.path.exists(whole):
        continue
    try:
        t = ast.parse(open(whole, encoding="utf-8").read())
    except SyntaxError:
        continue
    speaks = any(
...
from_git = 0
if not shipped:
    shipped = sorted("tests/" + os.path.relpath(os.path.join(at, n), HERE)
                     .replace(os.sep, "/")
                     for at, _folders, names in os.walk(HERE)
