...
Nothing under tests/resolve/live/ runs in the suite: those tests want a
...
FOLDER = os.path.join(HERE, "resolve", "live")
...
check("a run here names the Resolve tests that did not run in it",
      ...
      "resolve/live/ holds %d test files" % (len(opened), len(plain or ()),
                                             len(mine)))

named = int(opened[0].group(1)) if opened else -1
check("the run names as many Resolve tests as resolve/live/ holds files",
      named == len(mine),
