...
resolve/live/sweep.py deletes what resolve_ground.TEST_PROJECT matches
and nothing else, so a test that names its project or its folder any
other way leaves it standing in somebody's project manager. The sections in
...
def suite():
    ...
    `tests/resolve/live/` is never run there at all -- but its files lie
...
ALLOWED = {
    ("resolve/live/project_run_puts_back_test.py", "vpm-test-decoy-KEEP-%d"):
