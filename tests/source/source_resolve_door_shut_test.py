...
are started by hand. How many may be excepted is a ratchet.
...
ME = os.path.relpath(os.path.abspath(__file__), HERE).replace(os.sep, "/")
...
SIBLING = "source/source_test_names_swept_test.py"
GROUND = "resolve/live/resolve_ground.py"
...
def suite():
    ...
    and `tests/resolve/live/` never runs there at all -- but those files lie
    ...
    for root, folders, names in os.walk(HERE):
        ...
        for name in names:
            ...
            # resolve\live\sweep.py and git says resolve/live/sweep.py, and the
...
ALLOWED = {
    "resolve/live/project_clips_land_right_test.py":
