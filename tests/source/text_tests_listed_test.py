...
of them, and this holds the written list against the folder:
  ...
  6. the same for the tests under resolve/live/, in a table of their own
  7. the index names each test under the folder it lies in, and no
     name lies in two folders
...
# The tests under resolve/live/ are held apart, in section 6: they are not in
...
print("\n6. And the tests under resolve/live/ in a table of their own")
...
check("the tests under resolve/live/ are found at all", len(apart_here) > 0,
      "%d found in %s"
      % (len(apart_here), os.path.join(HERE, *overview.APART.split("/"))))
missing = sorted(set(apart_here) - set(apart_listed))
check("every test under resolve/live/ has a row in its own table",
      not missing,
      "%d of %d without one: %s"
      % (len(missing), len(apart_here), ", ".join(missing)))
gone = sorted(set(apart_listed) - set(apart_here))
check("every resolve/live/ row names a test under resolve/live/", not gone,
      "%d name none: %s" % (len(gone), ", ".join(gone[:4])))
...
check("every resolve/live/ row repeats its test's own first line",
      not apart,
      "%d differ -- %s" % (len(apart), " ; ".join(apart[:2])))

print("\n7. And the index says where each one lies")
# run.sh finds a test by its name in whichever folder it lies, so a name
# in two folders runs one of the two and drops the other without a word.
places = overview.test_places(HERE)
names = [name for name, _path in places]
doubled = sorted("%s in %s" % (name, " and ".join(
    path for n, path in places if n == name))
    for name in set(n for n in names if names.count(n) > 1))
check("no test name lies in two folders", not doubled,
      "%d names twice: %s" % (len(doubled), " ; ".join(doubled[:2])))
lies = overview.folders(HERE)
indexed = overview.folders_in(text)
off = sorted("%s: the index says %r, it lies in %r"
             % (name, said or "tests/", lies.get(name, "nowhere") or "tests/")
             for name, said in indexed if lies.get(name) != said)
off += sorted("%s: not in the index" % name
              for name in set(lies) - set(n for n, _ in indexed))
check("every test stands in the index under its folder",
      not off and len(indexed) == len(lies),
      "%d of %d wrong, %d index entries -- %s"
      % (len(off), len(lies), len(indexed), " ; ".join(off[:2])))
