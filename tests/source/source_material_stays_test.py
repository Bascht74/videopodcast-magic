...
material it may read lies beside it, under tests/samples, and a run
...
a fresh folder, every file under samples carries the bytes it had
...
MATERIAL = os.path.join(os.path.dirname(SCRIPT), "samples")
...
def sums():
    """Every file under samples: its path, its sha256, its bytes."""
    found = {}
...
check("a run leaves every file under samples with the bytes it found",
      not moved,
...
if inside is None or inside.returncode != 0:
    ...
    check("after the run every file under samples is what HEAD holds",
          not told,
