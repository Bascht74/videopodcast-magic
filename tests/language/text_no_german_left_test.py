...
# The helpers beside the tests, and every test of the suite in the
# folder it lies in -- as far as it lies on this machine.
mine = sorted(set(f for f in os.listdir(HERE) if f.endswith(".py"))
              | set(path for _name, path in overview.test_places(HERE)
                    if os.path.exists(os.path.join(HERE, path))))
mine = [f for f in mine if f.rpartition("/")[2] not in DETECTORS]
