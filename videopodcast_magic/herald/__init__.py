...
def show_progress(text, share=None):
    # In a parallel batch this thread's progress goes into the shared bar
    # rather than onto a line of its own; three bars would be unreadable.
...
def progress_from_line(line, duration):
    ...
    Returns a number between 0 and 0.999, or None where the line says
    nothing about progress. Four places read this output the same way.
    """
...
class SharedProgressBar(object):
    ...
    def show(self, share):
        ...
        if PROGRAM.OUTPUT_SINK:
            PROGRAM.OUTPUT_SINK(line)
            return
        # Past the buffer, or the bar appears only once the file is done.
    ...
    def report(self, who, share):
        with self.lock:
            ...
            # At most 99 % while the run is not through: the last file
            # reports itself done early, and 100 % with lines still
            # arriving below looks like a hang.
            total = min(0.99, total)
            if abs(total - self.last_time) < 0.005:
                return          # nothing new, so no second line
            self.last_time = total
        # The bar in the footer wants the joint figure: several threads
        # reporting one at a time would jump to whichever is furthest on.
        step_report(total)
        self.show(total)
...
class Share(object):
    ...
    def report(self, share, text=None):
        # The bar runs jointly; every step still enters this file's report,
        # or the lines one knows from a sequential run would be missing.
...
class ProgressPlan(object):
    ...
    the steps report. It never goes back: a step added while the job
    runs lowers the arithmetic, and that reads as a fault. A step that
    cannot say how far it is creeps towards its own end, slowing as it
    approaches and never reaching it.
    """
    ...
    def begin(self, name, caption="", weight=1.0):
        ...
        For work that reports nothing until it is finished. Such a step
        may creep the whole way; one that does report stays near it.
        """
    ...
    def drop(self, names):
        ...
        worked on and holds the bar up for ever. Marking it done lies.
        """
    ...
    def creep(self, seconds, reach=0.93, half_life=30.0, lead=0.12,
        ...
        that has reported a figure may only creep a little past it; one
        that reports nothing may creep the whole way. Past the ceiling
        it goes on at a tenth of the speed, up to *beyond*, which reads
        as "nearly there, still working" rather than as a promise.
        """
        ...
        for name in self.began:
            ...
            # is asymptotic, so the stretch past it is else unreachable.
            if here < top - 0.001:
...
def running_from():
    ...
    DaVinci Resolve both set it elsewhere. __file__ is what was loaded.
    """
    try:
...
# The mark the window's own lines carry, so grep can pick them out.
GUI_MARK = "[GUI]"

# What a speaker reading leaves in the state; cleared together, or one
# project's reading is read back in the next.
...
# 4K material with long GOPs is most of a second away.
SPOT_ARRIVED_MS = 1000

# Waiting for a jump to land, as the cut player's Seeker does. On 18
# and 27 GB files a file falls back to its front 18 to 88 ms after
# reporting itself loaded: a seek is a request, not a command.
SEEK_HIT_MS = 350
SEEK_AGAIN_MS = 120
SEEK_PATIENCE_S = 5.0
SEEK_SETTLE_S = 0.5
...
def outside_what(cmd):
    ...
    # A python of its own runs the speaker separation, and "python3" says
    # nothing about what takes the minutes. The script is the name.
...
# Otherwise the last run of identical calls is never pushed out.
...
def outside_work(tool, about):
    ...
    them. Said even where it fails: four minutes were still spent.
    """
...
# would be said twice. Per thread: prework runs several at once.
...
class SaysWhenDone(_subprocess_popen):
    ...
    Started and finished are two lines: a long call is interesting while
    it runs, and four minutes must be tellable from four seconds.
    """
...
def watch_outside_calls():
    ...
    main(), or the replacement would reach into whoever imported it.
    """
...
def redirect_console():
    ...
    try:
        ...
        # handles on one file keep two write positions, and whichever is
        # behind writes over what the other put there.
