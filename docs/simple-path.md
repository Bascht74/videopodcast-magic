...
There are two modes, and not every switch works in both. `--help` says
which: `[simple path only]` or `[multitrack only]`. Both markers stay
English, even with `--lang de`.
...
There is no `--container`; it is always MOV.

What the simple path does just like multitrack:

- **Time window.** `--in-point` and `--out-point` work here too, in the
  notations listed in [multitrack.md](multitrack.md), section "Time
  window". The audio is trimmed; the picture stays whole and keeps its
  timecode.
...
520 MB per track and hour; `--no-single-tracks` leaves them out. Where
the mix comes back from auphonic.com with a different length than they
have -- a jingle prepended on the free tier -- they drop out by
...
Where the file names give the search nothing to go on, the blocks can be
named: `--together A B C` makes them one recording, in that order,
repeatable for several. Each name brings the blocks that already belong
to it.

In the interface it is a selector **belongs to** on the recording, next
to where a block can be set apart. `--apart` takes a block out of a
recording the search found, `--together` puts in one it did not. Both are
by hand, both beat the measurement, and a file set apart stays out even
of a group it was put into. Both are stored in the project.
...
5. Reassemble: picture untouched (`-c:v copy`), the new audio as the first
   track, the camera track behind it (`--no-camera-audio` leaves it out),
   both named, timecode kept.
